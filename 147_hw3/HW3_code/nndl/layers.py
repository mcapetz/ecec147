...
def affine_forward(x, w, b):
  ...
  out = xr.dot(w) + b
  # print("wxr", xr.dot(w).shape) # check this shape to see if h2 shape is even correct???
...
def affine_backward(dout, cache):
  ...
  # m = 
  # d = 3
  # m = THERE IS SMTH WRONG HERE!!! WITH THE NUNMBERS
  ...
  # dw = np.dot(x.reshape(x.shape[0], -1).T, dout) # (N, M) x (1 x N) should be (D, M) original
  xr = x.reshape(x.shape[0], -1)
